 ...
 * Class Gamed
 *
 * @package hrace009/perfect-world-api
 */
class Gamed
{
    public $ip;
...
{
    $remain = strlen($data) - $p;
    if ($remain <= 0) {
        \Log::error("cuint - Không đủ dữ liệu để đọc: còn 0 byte");
        return 0;
    }

    $firstByte = ord($data[$p]);
    ...
    if ($firstByte < 0x80) {
        ...
        if ($remain < 2) {
            \Log::error("cuint - Không đủ dữ liệu (2-byte): còn $remain byte");
            return 0;
        }
        $val = unpack("n", substr($data, $p, 2))[1] - 0x8000;
        $p += 2;
    } elseif ($firstByte < 0xE0) {
        if ($remain < 4) {
            \Log::error("cuint - Không đủ dữ liệu (4-byte): còn $remain byte");
            return 0;
        }
        $val = unpack("N", substr($data, $p, 4))[1] - 0xC0000000;
        $p += 4;
    } else {
        \Log::error("cuint - Không hỗ trợ định dạng này: $firstByte");
        $p += 1;
        return 0;
    }
    ...
    {
        if (@fsockopen(config('pw-config.server_ip', '127.0.0.1'), $port, $errCode, $errStr, 1)) {
            $sock = socket_create(AF_INET, SOCK_STREAM, SOL_TCP);
            socket_connect($sock, config('pw-config.server_ip', '127.0.0.1'), $port);
        ...
        foreach ($struct as $key => $val) {
            if (is_array($val)) {
                ...
                switch ($val) {
                    case 'int':
                        if (strlen($rb) < 4) {
                            \Log::error("int - {$key} Không đủ dữ liệu để unpack INT: còn " . strlen($rb) . " byte, cần 4");
                            return false;
                        }
                    ...
                    case 'int64':
                        if (strlen($rb) < 4) {
                            \Log::error("int64 - {$key} Không đủ dữ liệu để unpack INT: còn " . strlen($rb) . " byte, cần 4");
                            return false;
                        }
                    ...
                    case 'long':
                        if (strlen($rb) < 4) {
                            \Log::error("long - {$key} Không đủ dữ liệu để unpack INT: còn " . strlen($rb) . " byte, cần 4");
                            return false;
                        }
                    ...
                    case 'lint':
                        if (strlen($rb) < 4) {
                            \Log::error("lint - {$key} Không đủ dữ liệu để unpack INT: còn " . strlen($rb) . " byte, cần 4");
                            return false;
                        }
                    ...
                    case 'byte':
                        if (strlen($rb) < 4) {
                            \Log::error("byte - {$key} Không đủ dữ liệu để unpack INT: còn " . strlen($rb) . " byte, cần 4");
                            return false;
                        }
                    ...
                    case 'cuint':
                        if (strlen($rb) < 4) {
                            \Log::error("cuint - {$key} Không đủ dữ liệu để unpack INT: còn " . strlen($rb) . " byte, cần 4");
                            return false;
                        }
                    ...
                    case 'octets':
                        ...
                        if (strlen($rb) < 4) {
                            \Log::error("octets - {$key} Không đủ dữ liệu để unpack INT: còn " . strlen($rb) . " byte, cần 4");
                            return false;
                        }
                    ...
                    case 'name':
                        if (strlen($rb) < 4) {
                            \Log::error("name - {$key} Không đủ dữ liệu để unpack INT: còn " . strlen($rb) . " byte, cần 4");
                            return false;
                        }
                    ...
                    case 'short':
                        if (strlen($rb) < 4) {
                            \Log::error("short - {$key} Không đủ dữ liệu để unpack INT: còn " . strlen($rb) . " byte, cần 4");
                            return false;
                        }
                    ...
                    case 'lshort':
                        if (strlen($rb) < 4) {
                            \Log::error("lshort - {$key} Không đủ dữ liệu để unpack INT: còn " . strlen($rb) . " byte, cần 4");
                            return false;
                        }
                    ...
                    case 'float2':
                        if (strlen($rb) < 4) {
                            \Log::error("float2 - {$key} Không đủ dữ liệu để unpack INT: còn " . strlen($rb) . " byte, cần 4");
                            return false;
                        }
                    ...
                    case 'float':
                        if (strlen($rb) < 4) {
                            \Log::error("float - {$key} Không đủ dữ liệu để unpack INT: còn " . strlen($rb) . " byte, cần 4");
                            return false;
                        }
    ...
    public function unpackOctet($data, &$tmp)
{
    ...
    $octet = substr($data, $p, $size);
    $tmp += $p + $size;
    return bin2hex($octet); // hoặc base64_encode nếu thích
}
