...
<div class="category-tabs-section">
    <div class="category-tabs">
        <a href="?category=equipment" class="category-tab {{ $category == 'equipment' ? 'active' : '' }}">
            <i class="fa-solid fa-screwdriver-wrench"></i>
            <span>Trang bị</span>
        </a>
        <a href="?category=bag" class="category-tab {{ $category == 'bag' ? 'active' : '' }}">
            <i class="fa-solid fa-briefcase"></i>
            <span>Túi đồ</span>
        </a>
        <a href="?category=inventory" class="category-tab {{ $category == 'inventory' ? 'active' : '' }}">
            <i class="fa-solid fa-briefcase"></i>
            <span>Kho đồ</span>
        </a>
        <a href="?category=trades" class="category-tab {{ $category == 'trades' ? 'active' : '' }}">
...
<div class="account-section">
    <a href="" class="account-section-title">
        <i class="fa-solid fa-briefcase"></i> Trang bị [{{$char->name}}]
    </a>
    <a href="" class="">
        <i class="fa-solid fa-search"></i> Tra cứu thông tin nhân vật khác?
    </a>
    <style>
        /* Slot cơ bản */
        .equipment-slot {
    ...
    <div style="margin-top:50px">
        <div class="shop-container">
            ...
            <div class="character-info">
                ...
                <div class="row"><strong>Last login: <span class="char">{{date('Y-m-d H:i:s', $base["lastlogin_time"] +
                            7*3600)}}</span></strong></div>
                ...
                @if(isRoleOnline($char->char_id))
                <div class="row"><strong>Toạ độ: <span class="char">{{(int)$info['posx'] .", " . (int)$info['posz']}}
                            ({{ $char->map }})</span></strong></div>
                <div class="row"><strong>Tình trạng:</strong> <span class="status-{{$info[" hp"]=="0" ? "offline"
                        : "online" }}">{{$info["hp"] == "0" ? "Đã bị giết" : "Bình thường" }}</span></div>
                @endif
...
<div class="account-section">
    <a href="" class="account-section-title">
        <i class="fa-solid fa-briefcase"></i> Túi đồ [{{Auth::user()->char->name}}]
    </a>
    <div class="security-tips">
        ...
        <ul style="width: 100%;">
            <li> Túi đồ sẽ được cập nhật sau mỗi 10-15 phút</li>
        </ul>
    </div>
    <div style="margin-top:50px">

        @foreach ($bagData as $xx)
        <div class="bag-grid">
            @foreach ($xx as $item)
            <div class="bag-slot vip-glow quality-legendary" title="{{$item['name']}}">
                <img src="{{$item['image']}}" />
                <div class="bag-count">{{$item['count']}}</div>
            </div>
            @endforeach
        </div>
        @endforeach
...
<div class="account-section">
    <a href="" class="account-section-title">
        <i class="fas fa-rotate"></i> Kho [{{Auth::user()->char->name}}]
    </a>
    <div style="margin-top:50px">
        @foreach ($storeData as $xx)
        <div class="bag-grid">
            @foreach ($xx as $item)
            <div class="bag-slot vip-glow quality-legendary" title="{{$item['name']}}">
                <img src="{{$item['image']}}" />
...
<div class="account-section">
    <a href="" class="account-section-title">
        <i class="fas fa-rotate"></i> Lịch sử giao dịch
    </a>
    ...
    <div style="margin-top:50px">
        <table id="killers-table">
            ...
            <tbody>
                ...
                <tr>
                    <th>{{ \Carbon\Carbon::parse($trade->date)->format('d/m/Y H:i:s') }}</th>
                    <th>{{ $trade->from_char->name }}</th>
                    <th>{{ $trade->to_char->name }}</th>
                    <th>
                        <ul class="list-group">
                            @foreach ($trade->items as $it)
                            @if($it->item->name == "")
                            <li class="list-group-item">Không xác định</li>
                            @else
                            <li class="list-group-item"><img src="{{$it->item->image}}" title="{{$it->item->name}}">
                                (x{{$it->quantity}})</li>
                            @endif
                            @endforeach
                        </ul>
